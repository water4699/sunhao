...
<!--      <el-table-column label="父科目ID" align="center" prop="categoryId" />-->
...
<script>
import { listSubject, getSubject, delSubject, addSubject, updateSubject } from "@/api/my/subject"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        categoryId: [
          { required: true, message: "父科目ID不能为空", trigger: "blur" }
        ],
        name: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除科目编号为"' + subjectIds + '"的数据项？').then(function() {
