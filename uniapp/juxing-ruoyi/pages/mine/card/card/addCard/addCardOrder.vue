		...
		<view class="content">
			<view class="goods-card">
				...
				<view class="goods-detail">
					<view class="goods-name">{{ goodsInfo.cardName }}</view>
			...
			<view class="total-section">
				...
				<text class="total-price">¥{{ goodsInfo.price }}</text>
			</view>
		</view>
		<view class="footer">
...
<script>
	...
	export default {
		...
		onLoad(card) {
			// 获取商品ID参数
			console.log('接收到的参数:', card.id); // 应该能打印出 id
			this.id = card.id;
			this.init();
		...
		methods: {
			// 初始化数据
			init() {
				getOneCard(this.id).then(res => {
					var msg = res.data;
					console.log(msg);
					this.goodsInfo.cardName = msg.cardName;
